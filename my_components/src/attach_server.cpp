...
AttachServer::AttachServer(const rclcpp::NodeOptions options)
    : Node("attach_server_component", options), cart_detected_(false),
      publish_tf_(false) {

  srv_cbg_ =
      this->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
  approach_srv_ = this->create_service<GoToLoading>(
      "approach_shelf", std::bind(&AttachServer::srv_callback, this, _1, _2),
...
void AttachServer::srv_callback(const GoToLoading::Request::SharedPtr request,
  ...
  if (cart_detected_ && request->attach_to_shelf) {
    ...
    while (true) {
      ...
      if (std::abs(error_distance) < 0.04) {
    ...
    RCLCPP_INFO(this->get_logger(),
                "Final appraoch done the robot is underneath the shelf.");
  } else {
    response->complete = false;
    RCLCPP_WARN(this->get_logger(),
                "The cart is not detected finale approach canceled.");
  }
}
...
AttachServer::get_pos(const sensor_msgs::msg::LaserScan::SharedPtr msg,
                      const int ind) {
...
std::pair<float, float>
AttachServer::get_median(const std::pair<float, float> &first_pos,
